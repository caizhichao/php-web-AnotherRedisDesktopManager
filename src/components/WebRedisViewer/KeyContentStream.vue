...
export default {
  ...
  methods: {
    initShow(resetTable = true) {
      ...
      const maxId = this.lastId ? this.lastId : (this.maxId ? this.maxId : '+');
      ...
      this.client.xrevrangeBuffer([
        ...
        for (const stream of reply) {
          const content = {};
          const line = { id: stream[0], content, uniq: Math.random() };
          // add key value map
          for (let i = 0; i < stream[1].length; i += 2) {
            content[this.$util.bufToString(stream[1][i])] = this.$util.bufToString(stream[1][i + 1]);
          }
